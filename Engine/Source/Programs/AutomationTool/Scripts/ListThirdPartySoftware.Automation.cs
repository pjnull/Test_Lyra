...
class ListThirdPartySoftware : BuildCommand
{
	public override void ExecuteBuild()
	{
		...
		foreach(string Target in ParseParamValues(Params, "Target"))
		{
			...
			Result = Run(UnrealBuild.UBTExecutable, String.Format("{0} {1} -Mode=JsonExport -OutputFile=\"{2}\"", Target.Replace('|', ' '),  ProjectPath, OutputFile.FullName), Options: ERunOptions.Default);
