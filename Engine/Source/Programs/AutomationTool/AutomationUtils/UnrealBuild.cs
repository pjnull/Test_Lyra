...
namespace AutomationTool
{
	...
	public class UnrealBuild
	{
		...
		{
			List<string> CommonCommandLine = new List<string>(3);

			CommonCommandLine.Add("-NoUBTMakefiles"); // BuildConfiguration
			
			if (bDisableXGE)
			{
				CommonCommandLine.Add("-noxge"); // BuildConfiguration
			}

			int UHTTargetCount = Targets.Count(x => String.Equals(x.TargetName, "UnrealHeaderTool"));

			if (!AlwaysBuildUHT &&
				// Prevent UHT from being cleaned if it's not going to be (re)built
				UHTTargetCount == 0)
			{
				CommonCommandLine.Add("-nobuilduht"); // BuildConfiguration
			}

			List<string> CommandLine = new List<string>(CommonCommandLine.Count + Targets.Count());
			CommandLine.AddRange(CommonCommandLine);
			...
			foreach (BuildTarget Target in Targets)
			{
				...
				if (UHTTargetCount > 1 && String.Equals(Target.TargetName, "UnrealHeaderTool"))
				{
					// multiple UHT targets in a single build can trouble UBT, because generated Engine/Binaries/$Platform/UnrealHeaderTool.{version,modules} files end up being written by different commands
					// build them as we find them in the list of build targets
					CommandUtils.RunUBT(CommandUtils.CmdEnv, UBTExecutable, String.Join(" ", CommonCommandLine.Concat(TargetCommandLine)));
				}
				else
				{
