	...
	for (AActor* Actor : Level->Actors)
	{
			...
			( bAllActors || IsObjectSelectedForExport(Context, Actor) ) )
		{
			if (Actor->ShouldExport())
			{
				...
				Ar.Logf( TEXT("%sBegin Actor Class=%s Name=%s Archetype=%s'%s'%s%s%s%s%s") LINE_TERMINATOR, 
					FCString::Spc(TextIndent), *Actor->GetClass()->GetPathName(), *Actor->GetName(),
					*Actor->GetArchetype()->GetClass()->GetPathName(), *Actor->GetArchetype()->GetPathName(), *ParentActorString, *SocketNameString, *GroupActor, *GroupFolder, *CopyPasteId );
