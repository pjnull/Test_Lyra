...
struct CORE_API FUnixTime : public FGenericPlatformTime
{
	...
	static FORCEINLINE uint32 Cycles()
	{
		...
		return static_cast<uint32>(static_cast<uint64>(ts.tv_sec) * 1e6 + static_cast<uint64>(ts.tv_nsec) / 1000ULL);
	}
	...
	static FORCEINLINE uint64 Cycles64()
	{
		struct timespec ts;
		clock_gettime(CLOCK_MONOTONIC, &ts);
		return static_cast<uint64>(static_cast<uint64>(ts.tv_sec) * 1e7 + static_cast<uint64>(ts.tv_nsec) / 100ULL);
	}
