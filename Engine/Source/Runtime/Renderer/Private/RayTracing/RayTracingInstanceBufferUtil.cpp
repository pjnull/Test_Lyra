	...
	TArrayView<FVector4f> OutTransformData)
{
	...
	ParallelFor(NumSceneInstances, 
		...
		](int32 SceneInstanceIndex)
		{
			...
			const uint32 BaseDescriptorIndex = BaseUploadBufferOffsets[SceneInstanceIndex] + (bGpuSceneInstance ? 0 : NumNativeGPUSceneInstances) + (bCpuInstance ? 0 : NumNativeCPUInstances);
