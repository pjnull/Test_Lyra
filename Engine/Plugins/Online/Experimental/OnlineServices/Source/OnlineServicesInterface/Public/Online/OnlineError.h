...
class FOnlineError;
#define LOCTEXT_NAMESPACE "OSSError"
...
public:
	...
	{
		FString ErrorId = FString::Printf(TEXT("%s"), *ErrorCode::ToString(ErrorCode));

		const FOnlineError* LoopInner = GetInner();
		while (LoopInner != nullptr)
		{
			ErrorId += FString::Printf(TEXT("-%s"), *ErrorCode::ToString(LoopInner->ErrorCode));
	...
	{
		return ErrorCodeSystem(ErrorCode);
	}

	ErrorCodeType GetCategory() const
	{
		return ErrorCodeCategory(ErrorCode);
	}

	ErrorCodeType GetValue() const
	{
		return ErrorCodeValue(ErrorCode);
	}
