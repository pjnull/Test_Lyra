...
protected:
	...
	void AppendUsingCurrentSettings()
	{
		...
		auto MakeTriangle = [this, &CornerToVert, &UVIndices, &NormalIndices, &FaceCornerToVert, &GroupID]
		(int FaceCorner1, int FaceCorner2, int FaceCorner3)
		{
			int TriIndex = AppendTriangle(FaceCornerToVert[FaceCorner1], FaceCornerToVert[FaceCorner2], FaceCornerToVert[FaceCorner3]);
